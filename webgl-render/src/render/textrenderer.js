...
export default class TextRenderer extends ShaderProgram {
	...
	initBuffers() {
		...
		const quadPoints = new Float32Array([0, 0, 1, 0, 1, 1, 1, 1, 0, 0, 0, 1]);
	...
	clean() {
		...
		if (this.buffers.point) {
			gl.deleteBuffer(this.buffers.point);
			this.buffers.point = null;
		}
		if (this.buffers.position) {
			gl.deleteBuffer(this.buffers.position);
			this.buffers.position = null;
		}
		if (this.buffers.charSize) {
			gl.deleteBuffer(this.buffers.charSize);
			this.buffers.charSize = null;
		}
		if (this.buffers.texturePosition) {
			gl.deleteBuffer(this.buffers.texturePosition);
			this.buffers.texturePosition = null;
		}
	}
}
